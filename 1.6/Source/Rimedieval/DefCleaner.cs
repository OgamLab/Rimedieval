...
namespace Rimedieval
{
    public static class DefCleaner
    {
        ...
        public static bool IsOdysseyDef(this Def def)
        {
            if (def?.modContentPack?.Name != null && def.modContentPack.Name == "Odyssey")
            {
                return true;
            }
            return false;
        ...
        public static List<ResearchProjectDef> GetAllowedProjectDefs(this List<ResearchProjectDef> list)
        {
            if (RimedievalMod.settings.restrictTechToPreIndustrialOnly)
            {
                ...
                list = list.Where(x => x.IsOdysseyDef() || (x.techLevel <= TechLevel.Industrial && x != microElectronics && !x.ContainsTechProjectAsPrerequisite(microElectronics))).ToList();
            }
            else
            {
                list = list.Where(x => x.IsOdysseyDef() || x.techLevel <= TechLevel.Medieval).ToList();
            }
            return list;
        }
